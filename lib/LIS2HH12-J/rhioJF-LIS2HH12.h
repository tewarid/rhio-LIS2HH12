...
//******Operating Modes******
    ...
    void setFrequency(byte ODR); 
    void setAxis(byte ZYX); 
    void setActiveInactive(byte threshold, byte duration);
    ...
    void setanalogBandwidth(byte BW);
    void setdigitalLPF(byte LPFcutOff);
    void setIntDataready(char IntDataReady);
    void setPolarityINT(char H_Lint);
    void setFS(byte FS);
    void setSelfTest(byte mode);
...
//******High Pass Filter******
    void setIntHPFData(byte IntHPF);
    void setHPF(byte HPF);
    void setFDS(byte OutData);
    void setCutOffHPF(byte HPFcutOff);
...
//*****Inercial Interrupt*****
    void setIntG1Mode(byte IntMode);
...
private:
    ...
    uint8_t readRegister(byte regis);
    void readAccel(int* x0, int* y0, int* z0);
    void writeRegister(byte val, byte regis, byte bt);
    char readbit(byte regis,int loc);
